...
public class Pacman implements Controllable, PlayerPositionSubject {
    ...
    public void collideWith(Level level, Renderable renderable) {
        if (renderable instanceof NewGhost) {
            NewGhost ghost = (NewGhost)renderable;
            renderable = ghost.getSetState();
        }

        if (renderable instanceof Collectable) {
            if (renderable.getLayer() != Layer.INVISIBLE) {
                Collectable collectable = (Collectable) renderable;
                level.collect(collectable);
                collectable.collect();
                
                if (collectable.isPowerPellet()) {
                    level.handleFrightened();
                }
            }
        }
    }
    ...
    @Override
    public void reset() {
        this.kinematicState = new KinematicStateImpl.KinematicStateBuilder().setPosition(startingPosition).setSpeed(kinematicState.getSpeed()).build();
        this.boundingBox.setTopLeft(this.kinematicState.getPosition());
